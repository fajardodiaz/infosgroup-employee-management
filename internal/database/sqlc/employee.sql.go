...
const listEmployeesWithRel = `-- name: ListEmployeesWithRel :many
...
type ListEmployeesWithRelParams struct {
	Limit  int32 `json:"limit"`
	Offset int32 `json:"offset"`
}

type ListEmployeesWithRelRow struct {
	ID                int64          `json:"id"`
	EmployeeCod       string         `json:"employee_cod"`
	FullName          string         `json:"full_name"`
	Birth             time.Time      `json:"birth"`
...
func (q *Queries) ListEmployeesWithRel(ctx context.Context, arg ListEmployeesWithRelParams) ([]ListEmployeesWithRelRow, error) {
	rows, err := q.db.QueryContext(ctx, listEmployeesWithRel, arg.Limit, arg.Offset)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []ListEmployeesWithRelRow{}
	for rows.Next() {
		var i ListEmployeesWithRelRow
		if err := rows.Scan(
			&i.ID,
			&i.EmployeeCod,
			&i.FullName,
